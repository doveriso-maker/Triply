...
function questionnaireReady(profile:any){if(!profile||profile.questionnaire_version!==QUESTIONNAIRE_VERSION||profile.questionnaire_complete!==true)return false;const q=profile.questionnaire;if(!q||typeof q!=='object'||Array.isArray(q))return false;if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(String(q.customer_email||profile.customer_email||'').trim()))return false;const modules=q.modules;if(!modules||typeof modules!=='object'||Array.isArray(modules))return false;for(const m of MODULES)if(modules[m]!==true)return false;for(const k of REQUIRED){const v=q[k];if(v===undefined||v===null||String(v).trim()==='')return false}for(const k of RATINGS){const n=Number(q[k]);if(!Number.isInteger(n)||n<1||n>5)return false}return true}
